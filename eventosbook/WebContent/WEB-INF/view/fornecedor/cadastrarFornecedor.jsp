...
<body class="login">
	...
	<div class="content">
		...
		<form class="register-form-fornecedor"
			action="<%=BibliotecaUrl.getInstance().getUrl(Constantes.LINK_FORNECEDOR_CONTROLADOR, Constantes.LINK_EFETUAR_CADASTRO_FORNECEDOR)%>"
			method="post">
			...
			<c:if test="${usuarioJaCadastrado eq true}">
					...
					<button class="close" data-close="alert"></button>
						<span>
							Este fornecedor ja está cadastrado!!!
						</span>
					</div>
				</c:if>
			...
			<div class="form-group">
				...
				<div class="input-icon">
					<i class="fa fa-font"></i> <input
						...
						placeholder="Nome da Empresa" name="nome" onkeydown="focusProxElementoOnEnter(event, 'email');"/>
				</div>
			</div>
			<div class="form-group">
				...
				<div class="input-icon">
					<i class="fa fa-envelope"></i> <input
						class="form-control placeholder-no-fix" type="text"
						placeholder="Email" name="email" id="email" onkeydown="focusProxElementoOnEnter(event, 'categoria');"/>
				</div>
			</div>

			<div class="form-group">
				<label class="control-label visible-ie8 visible-ie9">Categoria
				</label>

				<div class="input-icon">
					<i class="fa fa-envelop"></i> <select
						class="form-control select2_category" name="valorSelect" id="categoria" onkeydown="focusProxElementoOnEnter(event, 'numeroDocumento');">
						<option value="">Selecione o tipo da categoria</option>
						<c:forEach items="${listaCategoriaFornecedor}" var="categoriaFornecedor">
							<option value="${categoriaFornecedor.codigo}">${categoriaFornecedor.descricao}</option>
						</c:forEach>
					</select>
				</div>
			</div>
			...
			<div class="form-group">
				<!--ie8, ie9 does not support html5 placeholder, so we just show field title for that-->
				<label class="control-label visible-ie8 visible-ie9">Nº Documento (CNPJ)</label>
				<div class="input-icon">
					<i class="fa fa-building-o"></i> <input
						class="form-control placeholder-no-fix" type="text"
						placeholder="Nº Documento (CNPJ)" name="numeroDocumento" id="numeroDocumento" 	onkeydown="focusProxElementoOnEnter(event, 't');"/>
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="input-icon">
					<i class="fa fa-lock"></i> <input
						...
						autocomplete="off" id="register_password" placeholder="Senha"
						name="senha" onkeydown="focusProxElementoOnEnter(event, 'confirmSenha');"/>
				</div>
			</div>
			<div class="form-group">
			...
			<div class="form-actions">
				...
				<button type="submit" id="register-submit-btn"
					class="btn blue-book pull-right">
